...
tvprogress:connect_signal(
    ...
    function()
        ...
        anime_command =
            string.format("alacritty --working-directory %s/Scripts/ani-cli -e ./ani-cli", os.getenv("HOME"))
        if tv.type == "anime" then
            awful.spawn(anime_command)
        else
            awful.spawn(tv_command)
        end
    end
)
